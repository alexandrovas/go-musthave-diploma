...
**Пайплайн загрузки** (`LoadConfig`):
1. YAML-файл конфигурации через `file.Provider` + `yaml.Parser` (отсутствие файла — не ошибка).
2. CLI-флаги через `posflag.Provider` (spf13/pflag → koanf).
3. Переменные окружения через `env.Provider` с трансформом: `RUN_ADDRESS` → `run_address` (нижний регистр, двойной underscore как разделитель вложенности).
